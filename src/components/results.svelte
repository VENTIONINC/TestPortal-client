<script>
  ...
  const groups = $derived.by(() => {
    ...
    for (const model of resultModels) {
      ...
      if (specMap.has(spec)) {
        const config = dateConfigs.find(
          (config) => config.date === rest.result.dateKey
        );
  ...
  const shownTotal = $derived(
    filteredResults.filter((r) => r.result.isActive).length
  );
  const selectedTotal = $derived(
    filteredResults.filter((r) => r.result.isSelected).length
  );
</script>
...
<div class="main-container">
    ...
    {#if sidebarExpanded}
      <div class="filter-group">
        ...
        <label
          >Review Status:
          <select
            bind:value={FilterParams.reviewStatus}
        ...
        <label
          >Error Message: <input
            type="text"
            bind:value={FilterParams.errorMessage}
            onchange={applyFilters}
          /></label
        >
      ...
      <div class="filter-group">
        ...
        <label
          >Spec ID: <input
            type="text"
            bind:value={FilterParams.specId}
            oninput={applyFilters}
          /></label
        >
        <label
          >Spec File: <input
            type="text"
            bind:value={FilterParams.specFile}
            oninput={applyFilters}
          /></label
        >
        <label
          >Spec Name: <input
            type="text"
            bind:value={FilterParams.specName}
            oninput={applyFilters}
          /></label
        >
      </div>
      ...
      <div class="filter-group">
        ...
        <label
          >Environment: <input
            type="text"
            bind:value={FilterParams.environment}
            oninput={applyFilters}
          /></label
        >
        <label
  ...
  <section class="content">
    <div class="card day-stats">
      <div class="row">
        {#each dateConfigs as day}
          <div
            class="day-toggle col button {day.isActive ? 'dark' : 'outline'}"
    ...
    <div class="pagination">
      <button onclick={prevPage} disabled={FilterParams.page === 1}
        >Previous</button
      >
      <span>Page {FilterParams.page} of {totalPages}</span>
      <button onclick={nextPage} disabled={FilterParams.page === totalPages}
        >Next</button
      >
    </div>
  </section>
